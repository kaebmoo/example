...
file = os.getenv('YOUR_CSV_FILE')

# อ่านข้อมูลจาก CSV หรือแหล่งข้อมูลอื่น ๆ
df = pd.read_csv(file, dtype=str)  # Replace with your actual file path
...
def search_contact(query):
    results = df[
        ...
        (df['ต.บริหาร'].astype(str).str.contains(query)) |
        (df['ส่วนงาน'].astype(str).str.contains(query)) |
        (df['ชื่อเต็มส่วนงาน'].astype(str).str.contains(query))
    ]
    return results
...
# อ่านค่า webhook_url จากไฟล์ .env
...
def webhook():
    ...
    if text.startswith('/register '):
        ...
        if not email.endswith('@example.com'):
            send_to_telegram(chat_id, 'กรุณาใส่อีเมลที่ถูกต้องและใช้ domain @example.com เท่านั้น')
