...
class OTPService:
    ...
    def generate_otp(self, phone_number, expiration=30):
        ...
        while True:
            ...
            for phone_number in list(self.otp_storage.keys()):
                ...
                if current_time - otp_data['created_at'] > otp_data['expiration']:
                    # ลบ OTP ที่หมดอายุ
                    del self.otp_storage[phone_number]
            # รอ 10 วินาที ก่อนตรวจสอบอีกครั้ง
            time.sleep(10)
...
time.sleep(1)  # รอให้ OTP หมดอายุ

# ตรวจสอบว่าถูกลบออกจาก storage หรือไม่
print(f"Is {phone_number} still in storage?: {phone_number in otp_service.otp_storage}")

# ยืนยัน OTP
time.sleep(1)  # รอเวลาเพื่อทดสอบ expiration
is_valid = otp_service.confirm_otp(phone_number, otp)
